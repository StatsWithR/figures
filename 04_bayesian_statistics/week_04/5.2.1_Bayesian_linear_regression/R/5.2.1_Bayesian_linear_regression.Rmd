...
A conjugate analysis would use a bivariate normal distribution for the coefficients conditional on $\sigma^2$.   We will instead use a "reference prior" that is a limiting version of the conjugate prior.

The reference prior is of the form that is a "uniform" distribution on the intercept and slope (both over the entire real line).  This is not a "proper" distribution, but is a limiting case of a normal distribution  for $\beta_1, \beta_0$, $$\beta_0 \mid \sigma^2 \sim N(0, \sigma^2/n_0)$$ independent of $$\beta_1 \mid \sigma^2 \sim N(0, \sigma^2/n_0)$$ where we let $n_0 \to 0$.  In addition let's reparameterize the  variance in terms of the precision, $1/\sigma^2 \equiv \phi$  and use a limiting version of a gamma prior on $\phi$, so that the joint prior distribution is $$p(\beta_0, \beta_1, \phi) \propto 1/\phi$$. Under this "improper" prior distribution, the posterior distribution of the coefficients is centered at the ordinary least squares estimates, e.g. $$\beta_1 \mid \sigma^2, \text{data } \sim N(\hat{\beta}_1), \sigma^2/SXX)$$ and  $$\beta_0 \mid \sigma^2, \text{data } \sim N(\hat{\beta}_0), \sigma^2(1/n + 1/SXX)$$ 
where $SXX = \sum_i (X_i - \bar{X})^2$.

Unconditional on $\sigma^2$,  the posterior distributions become a Student $t$ distribution with degrees of freedom $n - 2$.  The posterior credible intervals under this "reference" prior are in fact numerically equivalent to the confidence intervals from the classical frequentist analysis, providing a baseline for other Bayesian analyses with more informative prior distributions or perhaps other "objective" prior distributions, such as a student-t prior with 1 degree of freedom which is known as the Cauchy.
...
We can also use the classical OLS functions to obain 95% intervals for the mean at a point $$x$$ and the prediction at a point $$x$$.
