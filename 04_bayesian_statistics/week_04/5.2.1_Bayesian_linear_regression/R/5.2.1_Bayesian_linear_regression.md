...
The residuals, which provide an estimate of the fitting error, are equal to $\\hat{\\epsilon}\_i = y\_i = \\hat{y}\_i$ and are the difference between the observed *y*<sub>*i*</sub> and fitted values $\\hat{y} = \\hat{\\alpha} + \\hat{\\beta} x\_i$ where *x*<sub>*i*</sub> is the abdominal circumference for the *i*th male and are used for diagnostics as well as estimating *σ*<sup>2</sup> via the mean squared error, which is the sum of the squared errors
$$\\hat{\\sigma}^2 =  \\frac{\\sum \\hat{\\epsilon}\_i^2}{n-2}$$
 divided by the degrees of freedom, where the degrees of freedom are *n* − 2 or the number of observations adjusted for the number of parameters that we estimated in the regression. The MSE, $\\hat{\\sigma}^2$, maybe obtained from the output as the square of the entry labeled "residual standard error".
...
![](5.2.1_Bayesian_linear_regression_files/figure-markdown_github/resid-1.png)
...
$$Y\_i = \\alpah + \\beta X\_i + \\epsilon\_i$$
 with the assumption that the errors *ϵ*<sub>*i*</sub> are independent and identically distributed as normal random variables with mean zero and constant variance *σ*<sup>2</sup>. This assumption is exactly the same used for classical inference for testing and constructing confidence intervals. The diagnostic plots from frequentist model fitting for checking residuals, may be used to help check this assumption

``` r
plot(bodyfat.lm, which=2)
```

![](5.2.1_Bayesian_linear_regression_files/figure-markdown_github/qqplot-1.png)
...
A conjugate analysis would use a bivariate normal distribution for the coefficients conditional on *σ*<sup>2</sup>, where *σ* provides scaling in term of units for the response. This corresponds to *α* given *σ*<sup>2</sup> being normally distributed
*α* ∣ *σ*<sup>2</sup> ∼ *N*(*a*<sub>0</sub>, *σ*<sup>2</sup>*S*<sub>*α*</sub>/*n*<sub>0</sub>)
 with a mean *a*<sub>0</sub>, and a variance controlled by the parameter *S*<sub>*α*</sub>/*n*<sub>0</sub>) and *n*<sub>0</sub> reflecting a "prior" sample size that controls how concentrated our prior beliefs are about the prior mean. Similarly, *β* is normally distributed given *σ*<sup>2</sup>
*β* ∣ *σ*<sup>2</sup> ∼ *N*(*b*<sub>0</sub>, *σ*<sup>2</sup>*S*<sub>*β*</sub>/*n*<sub>0</sub>)
 with a mean of *b*<sub>0</sub> and variance that is *σ*<sup>2</sup>*S*<sub>*β*</sub>/*n*<sub>0</sub>. The covariance between *α* and *β* is *σ*<sup>2</sup>*S*<sub>*α*, *β*</sub>/*n*<sub>0</sub> where *S*<sub>*α*, *β*</sub> is parameter that describes a priori our beliefs about how *α* and *β* vary together. If *S*<sub>*a**l**p**h**a*, *b**e**t**a*</sub> is 0, then a priori *α* will be independent of *β*, conditional on *σ*<sup>2</sup>. To complete the conjugate specification, we would assign 1/*σ*<sup>2</sup> a gamma distribution
1/*σ*<sup>2</sup> ∼ *G*(*ν*<sub>0</sub>/2, *ν*<sub>0</sub>*σ*<sub>0</sub><sup>2</sup>/2)
 with *ν*<sub>0</sub> degrees of freedom. The hyper-parameter *σ*<sub>0</sub><sup>2</sup> is a prior estimate of sigma squared. Because of conjugacy, the posterior distributions will be normal-gamma, with simple rules to update the parameters, however this does require eliciting several hyper-paramters.

As an alternative or starting point, it is useful to provide a default or reference analysis as a starting point. The reference prior distribution is obtained as a limit of the normal distributions as those variance parameters go to infinity and is flat or uniform in *α* and *β*. The limit of the conjugate gamma prior, with prior degrees of freedom going to 0, provides the reference prior for *σ*<sup>2</sup> that we've used previously.

The joint prior distribution is written as
*p*(*β*<sub>0</sub>, *β*<sub>1</sub>, *ϕ*)∝1/*ϕ*
...
$$\\alpha \\mid \\sigma^2, \\text{data } \\sim N(\\hat{\\alpha}, \\sigma^2(1/n + 1/SXX)$$
 where $SXX = \\sum\_i (X\_i - \\bar{X})^2$ and
$$\\beta \\mid \\sigma^2, \\text{data } \\sim N(\\hat{\\beta}), \\sigma^2/SXX)$$
 are the marginal posterior distributions of the slope and intercept condional on *σ*<sup>2</sup>. Unconditional on *σ*<sup>2</sup>, the posterior distributions become Student *t* distribution
$$\\alpha \\text{data } \\sim t\_{n-2}(\\hat{\\alpha}, \\sigma^2(1/n + 1/SXX)$$
 and
$$\\beta \\mid  \\text{data } \\sim t\_{n-2}(\\hat{\\beta}), \\sigma^2/SXX)$$
...
We can use `lm` to obtain the OLS estimates but now interprete from a Bayesian perspective:
...
The columns labeled Estimate and SE are equivalent to the posterior means and standard deviations respectively, which you could use to manually form credible intervals.
...
We can also use the classical OLS functions to obain 95% intervals for the mean at a point
*x*
 and the prediction at a point
*x*
.
...
![](5.2.1_Bayesian_linear_regression_files/figure-markdown_github/unnamed-chunk-3-1.png)

Note in the above the legend has "CI" - in this case using CI for confidence interval and credible interval is correct. The difference comes down to the interpretation. Let's look at case 39.

``` r
pred.39 = predict(bodyfat.lm, newdata=bodyfat[39,], interval="prediction", level=.095)
```

Based on the data that we have seen, a Bayesian would expect that a man with waist circumference of 148.1 or inches should have bodyfat of 54.216 with a 95% chance that it is between 53.602 and 54.83 percent.
