...
``` r
qqnorm(residuals(bodyfat.lm))
```

![](5.2.1_Bayesian_linear_regression_files/figure-markdown_github/qqplot-1.png) where the quantiles of the residuals (estimated errors) are plotted against their theoretical values or what we would expect if the data were normally distributed.

Under a reference prior of the form that is a "uniform" distribution on the intercept and slope (both over the entire real line). This is not a "proper" distribution, but is a limiting case of a normal distribution for *β*<sub>1</sub>, *β*<sub>0</sub>,
*β*<sub>0</sub> ∣ *σ*<sup>2</sup> ∼ *N*(0, *σ*<sup>2</sup>/*n*<sub>0</sub>)
 independent of
*β*<sub>1</sub> ∣ *σ*<sup>2</sup> ∼ *N*(0, *σ*<sup>2</sup>/*n*<sub>0</sub>)
 where we let *n*<sub>0</sub> → 0. In addition lets reparameterize the variance in terms of the precision, 1/*σ*<sup>2</sup> ≡ *ϕ* and use a limiting version of a gamma prior on *ϕ*, so that the joint prior distribution is
*p*(*β*<sub>0</sub>, *β*<sub>1</sub>, *ϕ*)∝1/*ϕ*
. Under this "improper" prior distribution, the posterior distribution of the coefficients is centered at the ordinary least squares estimates, e.g.
$$\\beta\_1 \\mid \\sigma^2, \\text{data } \\sim N(\\hat{\\beta}\_1), \\sigma^2/SXX)$$
 and
$$\\beta\_0 \\mid \\sigma^2, \\text{data } \\sim N(\\hat{\\beta}\_0), \\sigma^2(1/n + 1/SXX)$$
 where $SXX = \\sum\_i (X\_i - \\bar{X})^2$.

Unconditional on *σ*<sup>2</sup>, the posterior distributions become a Student *t* distribution with degrees of freedom *n* − 2. The posterior credible intervals under this "reference" prior are in fact numerically equivalent to the confidence intervals from the classical frequentist analysis, providing a baseline for other Bayesian analyses with more informative prior distributions or perhaps other "objective" prior distributions, such as a student-t prior with 1 degree of freedom which is known as the Cauchy.

Using `lm` to obtain the OLS estimates.
